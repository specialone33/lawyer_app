                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.payments.update", [$payment->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('customer') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="customer_id" id="customer_id" required>
                                @foreach($customers as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('customer_id') ? old('customer_id') : $payment->customer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="title" id="title" value="{{ old('title', $payment->title) }}" required>
                            @if($errors->has('title'))
                        ...
                        <div class="form-group {{ $errors->has('payment_date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="payment_date" id="payment_date" value="{{ old('payment_date', $payment->payment_date) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('casefile') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="casefile_id" id="casefile_id">
                                @foreach($casefiles as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('casefile_id') ? old('casefile_id') : $payment->casefile->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('contract') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="contract_id" id="contract_id">
                                @foreach($contracts as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('contract_id') ? old('contract_id') : $payment->contract->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('companycontract') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="companycontract_id" id="companycontract_id">
                                @foreach($companycontracts as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('companycontract_id') ? old('companycontract_id') : $payment->companycontract->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('other_case') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="other_case_id" id="other_case_id">
                                @foreach($other_cases as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('other_case_id') ? old('other_case_id') : $payment->other_case->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
