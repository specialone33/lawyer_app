        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.foldernumber.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.foldernumbers.store") }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group {{ $errors->has('other_cases') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="other_cases_id" id="other_cases_id">
                                @foreach($other_cases as $id => $entry)
                                    <option value="{{ $id }}" {{ old('other_cases_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
