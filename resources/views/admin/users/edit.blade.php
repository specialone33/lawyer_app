                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('capacity') ? 'has-error' : '' }}">
                            <label class="required">{{ trans('cruds.user.fields.capacity') }}</label>
                            <select class="form-control" name="capacity" id="capacity" required>
                                ...
                                @foreach(App\Models\User::CAPACITY_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('capacity', $user->capacity) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('capacity'))
                                <span class="help-block" role="alert">{{ $errors->first('capacity') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.user.fields.capacity_helper') }}</span>
                        </div>
