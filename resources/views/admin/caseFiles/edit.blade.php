        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.caseFile.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.case-files.update", [$caseFile->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('case_file_number') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="case_file_number" id="case_file_number" value="{{ old('case_file_number', $caseFile->case_file_number) }}" step="1" required>
                        ...
                        <div class="form-group {{ $errors->has('registration_date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="registration_date" id="registration_date" value="{{ old('registration_date', $caseFile->registration_date) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('case_type') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="case_type_id" id="case_type_id" required>
                                @foreach($case_types as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('case_type_id') ? old('case_type_id') : $caseFile->case_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('case_description') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="case_description" id="case_description">{{ old('case_description', $caseFile->case_description) }}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('user') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="user_id" id="user_id" required>
                                @foreach($users as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $caseFile->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('user'))
                        ...
                        <div class="form-group {{ $errors->has('court') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="court_id" id="court_id" required>
                                @foreach($courts as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('court_id') ? old('court_id') : $caseFile->court->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('court'))
                        ...
                        <div class="form-group {{ $errors->has('procedural_process') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="procedural_process_id" id="procedural_process_id" required>
                                @foreach($procedural_processes as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('procedural_process_id') ? old('procedural_process_id') : $caseFile->procedural_process->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('hearing') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="hearing" id="hearing" value="{{ old('hearing', $caseFile->hearing) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('case_status') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="case_status_id" id="case_status_id" required>
                                @foreach($case_statuses as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('case_status_id') ? old('case_status_id') : $caseFile->case_status->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('next_actions') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="next_actions" id="next_actions" value="{{ old('next_actions', $caseFile->next_actions) }}">
                        ...
                        <div class="form-group {{ $errors->has('customers') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="customers[]" id="customers" multiple required>
                                @foreach($customers as $id => $customer)
                                    <option value="{{ $id }}" {{ (in_array($id, old('customers', [])) || $caseFile->customers->contains($id)) ? 'selected' : '' }}>{{ $customer }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('opponent') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="opponent" id="opponent" value="{{ old('opponent', $caseFile->opponent) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('lawyer') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="lawyer_id" id="lawyer_id" required>
                                @foreach($lawyers as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('lawyer_id') ? old('lawyer_id') : $caseFile->lawyer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('comments') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="comments" id="comments">{{ old('comments', $caseFile->comments) }}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('petition_filing_number') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="petition_filing_number" id="petition_filing_number" value="{{ old('petition_filing_number', $caseFile->petition_filing_number) }}">
                        ...
                        <div class="form-group {{ $errors->has('decision_number') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="decision_number" id="decision_number" value="{{ old('decision_number', $caseFile->decision_number) }}">
                        ...
                        <div class="form-group {{ $errors->has('charging_expenses') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="charging_expenses" id="charging_expenses" value="{{ old('charging_expenses', $caseFile->charging_expenses) }}" step="0.01">
                        ...
                        <div class="form-group {{ $errors->has('hours') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="hours" id="hours" value="{{ old('hours', $caseFile->hours) }}" step="1">
                            @if($errors->has('hours'))
                        ...
                        <div class="form-group {{ $errors->has('hourly_rate') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="hourly_rate" id="hourly_rate" value="{{ old('hourly_rate', $caseFile->hourly_rate) }}" step="0.01">
                        ...
                        <div class="form-group {{ $errors->has('one_time_fees') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="one_time_fees_id" id="one_time_fees_id">
                                @foreach($one_time_fees as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('one_time_fees_id') ? old('one_time_fees_id') : $caseFile->one_time_fees->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('custom_one_time_fee_name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="custom_one_time_fee_name" id="custom_one_time_fee_name" value="{{ old('custom_one_time_fee_name', $caseFile->custom_one_time_fee_name) }}">
                        ...
                        <div class="form-group {{ $errors->has('custom_one_time_fee_value') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="custom_one_time_fee_value" id="custom_one_time_fee_value" value="{{ old('custom_one_time_fee_value', $caseFile->custom_one_time_fee_value) }}" step="0.01">
